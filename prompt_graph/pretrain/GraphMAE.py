...
class GraphMAELoss(nn.Module):
    ...
    def encoding_mask_noise(self, g, x, mask_rate=0.3):
        ...
        if self._replace_rate > 0:
            ...
            # print(noise_to_be_chosen.shape)
            # print(noise_nodes.shape)
            # print(x.shape)
            # print(out_x.shape)
...
class GraphMAE(PreTrain):
    def __init__(self, *args,mask_rate=0.75, drop_edge_rate=0.0, replace_rate=0.1, loss_fn='sce', alpha_l=2, **kwargs):    # hid_dim=16
        ...
        self.node_fea_dim =  self.input_dim
        self.input_dim = 100
        self.graph_n_feat_dim = self.input_dim
        self.initialize_gnn(self.input_dim, self.hid_dim)
        self.embedding =  torch.nn.Sequential(torch.nn.Linear(self.node_fea_dim, 100, bias = False)).to(self.device)
        self.decoder = initialize_gnn_decoder(self.gnn_type,self.hid_dim,self.input_dim,self.num_layer,self.device)
        ...
        self.optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, list(self.gnn.parameters()) + list(self.decoder.parameters())+ list(self.embedding.parameters())),
